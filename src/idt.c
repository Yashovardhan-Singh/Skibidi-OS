...
void IDTInit() {
    ...
    for (int i = 0; i < 256; i++) {
        IDTSetGate(i, (u32)isr_table[i], 0x08, 0x8E); // 0x08 = code segment
    }

    // IDTSetGate(0, (u32) isr32, 0x08, 0x8e);

    __asm__ volatile ("lidt %0" : : "m" (idtp));

    // IDTPointer temp;
    // asm ("sidt %0" : "=m" (temp));
    // if (temp.limit != 0x7FF || temp.base != (u32)&idt) {
    //     asm ("hlt"); // Halt if IDT didn’t load
    // }
}
